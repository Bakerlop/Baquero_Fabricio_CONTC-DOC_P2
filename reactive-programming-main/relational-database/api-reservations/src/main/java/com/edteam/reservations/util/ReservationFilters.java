...
public final class ReservationFilters {
    ...
    public static final Predicate<ReservationEvent> VALID_RESERVATION = event -> event.getPrice() != null
            && event.getPrice() > 0 && event.getEmails() != null && !event.getEmails().isEmpty();

    public static final Consumer<ReservationEvent> PRINT_PROCESSED_EVENT = event -> System.out
            .println("Evento procesado: " + event);
}